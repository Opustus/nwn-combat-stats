...
#' @param base_apr Numeric. Base Attacks Per Round (APR) derived from BAB.
#' @param ubab Logical. True if using Monk Unarmed Base Attack Bonus progression (-3 decrement instead of -5).
#'
...
#' @param ubab Logical. Uses Monk unarmed progression.
#' @param haste Logical. Adds one attack at highest AB.
#' @param flurry Logical. Adds one attack at highest AB, applies -2 penalty to all attacks.
#' @param dualwield Logical. Adds two off-hand attacks, applies -2 penalty to all attacks.
...
attacks_df <- function(ab, base_apr, ubab = FALSE, haste = TRUE, flurry = FALSE, dualwield = FALSE, extra_apr = FALSE) {
  ...
  if (extra_apr) df$extra_apr <- attacks(ab, base_apr, ubab)[1] + roll20
...
hits <- function(ab, base_apr, ubab, haste, flurry, dualwield, extra_apr, enemy_ac) {
  hit_rolls <- attacks_df(ab, base_apr, ubab, haste, flurry, dualwield, extra_apr) - enemy_ac
...
crits <- function(ab, base_apr, ubab, haste, flurry, dualwield, extra_apr, enemy_ac, crit_range) {
  crit_rolls <- attacks_df(ab, base_apr, ubab, haste, flurry, dualwield, extra_apr) - enemy_ac
  ...
  crits <- hits(ab, base_apr, ubab, haste, flurry, dualwield, extra_apr, enemy_ac) * crit_chance
  return(crits)
}
...
damage <- function(ab, base_apr, ubab, haste, flurry, dualwield, extra_apr, enemy_ac, crit_range, crit_threat, dmg_per_hit, sneak_per_hit) {
  crits_val <- crits(ab, base_apr, ubab, haste, flurry, dualwield, extra_apr, enemy_ac, crit_range)
  hits_val <- hits(ab, base_apr, ubab, haste, flurry, dualwield, extra_apr, enemy_ac) - crits_val
...
#'                 (ab, base_apr, ubab, haste, flurry, dualwield, extra_apr, crit_range, crit_threat, dmg_per_hit, sneak_per_hit).
