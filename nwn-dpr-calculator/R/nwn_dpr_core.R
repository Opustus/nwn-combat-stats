...
#' @param ferocity Logical. Adds one attack at highest AB.
#'
...
attacks_df <- function(ab, base_apr, ubab = FALSE, haste = TRUE, flurry = FALSE, dualwield = FALSE, ferocity = FALSE) {
  ...
  if (ferocity) df$ferocity <- attacks(ab, base_apr, ubab)[1] + roll20
...
#' @param ferocity Logical. Ferocity (+1 APR) active.
...
#'                 (ab, base_apr, ubab, haste, flurry, dualwield, ferocity, crit_range, crit_threat, dmg_per_hit, sneak_per_hit).
...
calculate_dpr_for_build <- function(row_data, ac_sequence) {
  sapply(ac_sequence, function(enemy_ac) {
    damage(
      ...
      ferocity = as.logical(row_data$ferocity),
      enemy_ac = enemy_ac,
